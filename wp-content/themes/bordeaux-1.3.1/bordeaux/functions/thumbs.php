...
function get_post_thumb($post_id,$width,$height,$custom="image") {		//universal thumb function
	...
	} elseif($first_from_post != false && $custom!="bordeaux_slider_image") {		//first attached image
...
{
	global $post;
	$img = get_post_thumb($post->ID,170,170);
	if($img['show'] != false) {
		if($img['src'] != "") {
			$img = '<img src="'.$img['src'].'" alt="article-image" class="post-image-1"  width="170" height="170"/>';
			return $img." ".$content;
		} else {
			return $content;
		}
...
function add_image_thumb_2($content)	//add thumb in the begining of the post
{
	global $post;
